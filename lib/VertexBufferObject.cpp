...
/**
 * Sets up and enables the vertex attribute pointers for the given vertex format.
 *
 * @tparam VertexFormat
 */
...
void setVertexAttribs<Vertex2DRgb>() {
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex2DRgb), (GLvoid *)(sizeof(float) * 2));
    glEnableVertexAttribArray(0);
...
void setVertexAttribs<Vertex3DRgb>() {
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex3DRgb), (GLvoid *)(sizeof(float) * 3));
    glEnableVertexAttribArray(0);
    glEnableVertexAttribArray(1);
}
...
template <typename VertexFormat>
VertexBufferObject<VertexFormat>::VertexBufferObject(
    const std::vector<VertexFormat> &vertices, const std::vector<GLuint> &indices) {
	m_indexed = true;
